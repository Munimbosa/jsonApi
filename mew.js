...
async function getOrCreateBankData(bankDataPath, user) {
...
async function handleDeposit(message, user, amount, bankData, usersData, senderID) {
...
async function handleWithdraw(message, user, amount, bankData, usersData, senderID) {
  const userMoney = await usersData.get(senderID, "money");
  const bankBalance = bankData[user].bank || 0;
...
async function handleBalance(message, user, bankData, senderID) {
  const bankBalance = bankData[user].bank || 0;
...
async function handleInterest(message, user, bankData, senderID) {
...
async function handleTransfer(message, user, amount, recipientUID, bankData, senderID) {
  if (isNaN(amount) || amount <= 0) {
...
async function handleLoan(message, user, amount, bankData, senderID) {
...
async function handleLoanPayment(message, user, amount, bankData, usersData, senderID) {
...
async function handleRichest(message, bankData) {
